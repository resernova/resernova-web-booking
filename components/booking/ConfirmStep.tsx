...
import { CASABLANCA_TZ, formatDate, formatSlot } from "@/lib/utils/time";
...
const labels = {
  fr: { summary: "Récapitulatif", service: "Prestation", date: "Date", time: "Heure", duration: "Durée", price: "Prix", total: "Total", whatsappConfirmed: "Rappel WhatsApp activé", noPreference: "Aucune préférence", back: "Retour" },
  en: { summary: "Summary", service: "Service", date: "Date", time: "Time", duration: "Duration", price: "Price", total: "Total", whatsappConfirmed: "WhatsApp reminder on", noPreference: "No preference", back: "Back" },
  ar: { summary: "الملخص", service: "الخدمة", date: "التاريخ", time: "الوقت", duration: "المدة", price: "السعر", total: "المجموع", whatsappConfirmed: "تذكير واتساب مفعل", noPreference: "دون تفضيل", back: "رجوع" },
};

export function ConfirmStep(props: Props) {
  const { form, serviceName, serviceDurationMinutes, servicePrice, selectedSlot, selectedDate, staffList, submitting, onBack, onSubmit, backLabel, confirmLabel, submittingLabel, locale } = props;
  const t = labels[locale];
  const values = form.watch();

  const staffName = staffList.find((s) => s.id === values.requestedStaffId)?.name ?? t.noPreference;
  const slotTime = selectedSlot ? formatSlot(selectedSlot.start, locale === "ar" ? "ar-MA" : locale === "en" ? "en-MA" : "fr-MA") : "";
  const slotDate = selectedDate ? formatDate(selectedDate + "T00:00:00", locale === "ar" ? "ar-MA" : locale === "en" ? "en-MA" : "fr-MA") : "";
  ...
  return (
    <div>
      ...
      <dl className="mt-4 space-y-3 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface-muted)] p-5 text-sm">
        ...
        <Row label={staffList.length > 0 ? locale === "fr" ? "Praticien" : locale === "en" ? "Staff" : "الممارس" : ""} value={staffName} />
      ...
      <div className="mt-8 flex items-center justify-between">
        ...
        >
          <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M13 5l-5 5 5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
          {backLabel}
        </button>
        <button
          type="button"
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <dd className={bold ? "font-display text-lg font-semibold" : "font-medium"}>{value}</dd>
    </div>
  );
}
